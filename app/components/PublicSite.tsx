...
function Header({ page }: { page: Page }) {
  ...
  return <header className="public-header"><Link href="/"><MentorMeLogo /></Link>
    <nav>{nav.map(n => <Link className={page === n.page ? "active" : ""} href={n.href} key={n.page}>{n.label}</Link>)}</nav>
    <div className="header-actions"><div className="notice-wrap"><button className="icon-btn bell" onClick={() => setNotice(!notice)} aria-label="Notifications">♢<i /></button>
      ...
      <ThemeToggle /><Link className="login-link" href="/login">Log in</Link><Link className="btn btn-small" href="/signup">Start free</Link></div>
